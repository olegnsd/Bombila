...
function getOrderState( $phone, $order_id ) {
  ...
  for( $i=0; $i<count($orders); $i++ ) {
    ...
    if( $orders[$i]['id'] == $order_id && $orders[$i]['passenger'] == $phone ) {
...
function declineOrder( $phone, $order_id ) {
  ...
  for( $i=0; $i<count($orders); $i++ ) {
    if( $orders[$i]['id'] == $order_id ) {
      $orders[$i]['state'] = 'declined';
      $orders[$i]['bombila'] = $phone;
      $orders[$i]['accept_time_secs'] = 0;
      $orders[$i]['accept_time_limit_secs'] = 0;
      break;
    }
  }
...
function getSuggestedOrder( $phone ) {
  ...
  foreach( $orders as &$order ) {
    if( $order['bombila'] == $phone && $order['state'] == 'suggested' ) {
...
function initMemcached() {
  ...
  $memcache->connect('/tmp/memcached', 0);
  if( !$memcache ) {
...
function sendBombilaLocation( $data ) {
  ...
  $rows = $memcache->get('bombilas_locations');
  ...
  foreach( $rows as &$row ) {
    ...
    if( $row['phone'] == $data['phone'] ) {
      ...
      $row['barter_coin'] = $data['barter_coin'];
      $row['childish_armchair'] = $data['childish_armchair'];
  ...
  $memcache->set('bombilas_locations', $rows, false, 0 );
}
...
function getNearBombilasLocations( $latitude, $longitude ) {
  ...
  foreach( $bombilas as &$bombila ) {
    ...
    $distance_meters = vincentyGreatCircleDistance( $latitude, $longitude, $bombila['latitude'], $bombila['longitude'] );
