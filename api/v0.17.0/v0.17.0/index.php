...
";
  ...
  $stmt->bindParam(':tariff_km',                             $order['tariff_km']);
  $stmt->bindParam(':tariff_minute',                         $order['tariff_minute']);
  $stmt->bindParam(':commission_percent',                    $order['commission_percent']);
  $stmt->bindParam(':route_length_km',                       $order['route_length_km']);
  $stmt->bindParam(':route_time_secs',                       $order['route_time_secs']);
  $stmt->bindParam(':get_in_price',                          $order['get_in_price']);  
  $stmt->bindParam(':waiting_price',                         $order['waiting_price']);    
  $stmt->bindParam(':minimal_price',                         $order['minimal_price']);    
...
function acceptOrder( $phone, $order_id ) {
  ...
  $query = "UPDATE orders set bombila=:phone, state='accepted' where id=:order_id and :time <= accept_time_limit_secs limit 1"; 
...
limit 1
  ...
  error_log("!!!! $stmt->rowCount(): ".$stmt->rowCount() ); 
...
latitude=:latitude, 
longitude=:longitude, 
orders_distance_km=:orders_distance_km,
...
";
  ...
  $stmt->bindParam(':orders_distance_km', $data['orders_distance_km']);  
...
function syncPriceOnStart( $phone, $order_id, $price ) {
  global $database;
