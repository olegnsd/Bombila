...
function setAutoPhoto( $phone ) {
  ...
  if( !$result ) {
    http_response_code(500);    
    error_log("Failed to save file: " . mysqli_connect_error());
    exit(0);    
  }
...
function getSuggestedOrder( $phone ) {
  ...
  while($row = mysqli_fetch_assoc($result)) {
    ...
    $order['not_to_call'] = boolval( $row['not_to_call'] );
    $order['wait'] = boolval( $row['wait'] );
    $order['not_to_smoke'] = boolval( $row['not_to_smoke'] );
    $order['childish_armchair'] = boolval( $row['childish_armchair'] );
...
";
  ...
  $stmt = mysqli_prepare($connection, $query);
  mysqli_stmt_bind_param($stmt, "sddiisiddiisi", 
    $data['phone'], 
    $data['latitude'], 
    $data['longitude'],
    $data['barter_coin'],
