...
function doJobMySql() {
  ...
  $bombilas = $stmt->fetchAll();
	...
	// мы удаляем те order_sync_data, котоыре по какой-то причине зависли в базе
	// и не были удалены стандартным путём
	removeOldOrdersSyncData();
...
function findNearestBombila( $bombilas, $order, $bombila_phone = NULL ) {
  ...
  $bombila_orders_distance_meters = 0;
  ...
  foreach( $bombilas as &$bombila ) {
    ...
    $distance_meters = vincentyGreatCircleDistance( $order['from_latitude'], $order['from_longitude'], $bombila['latitude'], $bombila['longitude'] );
		...
		$bombila_orders_distance_meters = doubleval( $bombila['orders_distance_km'] ) * 1000;

    if( $distance_meters <= $bombila_orders_distance_meters && $distance_meters < $min ) {
      $min = $distance_meters;
      $nearest = $bombila;
      // break;
    }
...
function removeOldOrdersSyncData() {
	global $database;

	$now = time();
	$lifetime_secs = 3600 * 24 * 7; // старше 1 недели

  $query = "DELETE from orders_sync where sync_time + :lifetime_secs <= :now";
